...
function commitWork(fiber) {
  ...
  // 하지만 함수형 컴포넌트 경우 dom이 없으므로
  // dom이 있는 부모가 나올 때까지 fiber 트리 상단으로 올라가고 걔랑 연결해줌
...
function useState(initial) {
  ...
  const newStates = oldHook ? oldHook.queue : [];
  newStates.forEach((newState) => (hook.state = newState));
